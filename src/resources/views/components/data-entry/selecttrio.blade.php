<div class="section-row">
    @if($name != null)
        @php
            ...
            if(isset($selected) && $selected != null && !is_null($attributes['multiple']))
            {
                $selected = array_fill_keys($selected, 'ids');
            }
        ...
        @endif
                ...
                @if($picks || $api)
                        ...
                        @if($picks) 
                            @foreach ($picks as $pick)
                                @if (isset($selected) && $selected != null && is_null($attributes['multiple']))
                                    ...
                                    @endphp
                                @elseif(isset($selected) && $selected != null && !is_null($attributes['multiple']))
                                    <option value="{{ $pick['id'] }}" data-uuid={{$pick['uuid']}} {{ (array_key_exists($pick['id'],$selected)) ? "selected" : "" }}>{{ $pick['text'] }}</option>
                                    @php 
                                        if(array_key_exists($pick['id'],$selected)){
                                            $multiSelectedId[]   = $pick['id'];
                                            $multiSelectedUuid[]   = $pick['uuid'];
                                            $multiSelectedTitle[]  = $pick['text'];
                                        }
                                    @endphp
                                @else
                    ...
                    @elseif(isset($selected) && $selected != null && !is_null($attributes['multiple']))
                        <input type="{{$showHidden == true ? 'text' : 'hidden'}}" class="{{$name}}-id-input" name="{{$nameId}}" value="{{implode(',', $multiSelectedId)}}">
                        <input type="{{$showHidden == true ? 'text' : 'hidden'}}" class="{{$name}}-uuid-input" name="{{$nameUUID}}" value="{{implode(',', $multiSelectedUuid)}}">
                        <input type="{{$showHidden == true ? 'text' : 'hidden'}}" class="{{$name}}-title-input" name="{{$nameTitle}}" value="{{implode(',', $multiSelectedTitle)}}">
                    @else
